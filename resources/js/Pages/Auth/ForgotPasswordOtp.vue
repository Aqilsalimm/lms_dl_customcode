...
const resendOtp = () => {
    resendForm.post(route('password.otp.resend'));
};
...
<template>
    <Head title="Verifikasi Kode OTP | Drastha Learning" />
    ...
    <div class="min-h-screen w-full flex flex-col md:flex-row bg-[#F4F7F9] font-montserrat relative overflow-hidden">
        ...
        <div class="flex-grow flex flex-col items-center justify-center p-6 sm:p-12 md:p-16 xl:p-24 relative min-h-screen">
            <!-- Status Alerts -->
            <div v-if="status" class="mb-6 text-xs font-bold text-green-600 bg-green-50 p-4 rounded-2xl border border-green-100 max-w-[500px] w-full">
                {{ status }}
            </div>

            <div v-if="resendForm.processing" class="mb-6 text-xs font-bold text-blue-600 bg-blue-50 p-4 rounded-2xl border border-blue-100 max-w-[500px] w-full">
            ...
            <div class="bg-white rounded-[2rem] p-8 sm:p-12 w-full max-w-[500px] shadow-[0_12px_50px_rgba(0,0,0,0.025)] border border-slate-100/50 flex flex-col gap-6 transition-all duration-300">
                <div class="flex flex-col items-center text-center">
                    ...
                    <h2 class="text-2xl sm:text-3xl font-extrabold text-[#1A2B49] leading-tight mb-4">Verifikasi OTP Lupa Password</h2>
                </div>

                <!-- Info Box matching login OTP screen -->
                <div class="bg-[#F9CC6B] rounded-2xl p-5 flex flex-col gap-2 text-left">
                    <p class="text-[#1A2B49] text-[11px] sm:text-xs font-bold leading-relaxed">
                        Kode OTP lupa password telah dikirim ke alamat email anda :<br />
                        <span class="text-[#264790] font-extrabold break-all">{{ email }}</span>
                    </p>
                    <p class="text-[#1A2B49] text-[10px] sm:text-[11px] font-bold leading-relaxed">
            ...
            <div class="flex flex-col gap-6 items-center mt-8">
                ...
                <Link 
                  ...
                  class="text-xs font-extrabold text-[#264790] hover:text-[#44A6D9] transition-colors mt-2"
                >
                    Kembali
                </Link>
            </div>
